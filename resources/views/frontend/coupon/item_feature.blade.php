<div class="tw-flex-shrink xl:tw-w-1/2 tw-w-full @if($index == 0)xl:tw-mr-6 @endif">
    <div dusk="coupon-card" class="tw-cursor-auto">
        <div class="tw-rounded-sm tw-border tw-bg-white sm:tw-p-4 tw-p-2 sm:tw-mb-4 tw-mb-2 xl:tw-h-full">
            <div class="tw-flex lato tw-h-full">
                @if(!empty($show_logo) && $coupon->store)
                    <div class="tw-text-center xl:tw-w-1/4 tw-w-1/4 xl:tw-min-w-1/6 tw-min-w-1/4">
                ...
                @else
                    <div class="tw-text-center xl:tw-w-1/4 tw-w-1/4 xl:tw-min-w-1/6 tw-min-w-1/4">
                        <a href="{{ $coupon->store->getUrl() }}" target="_blank">
                            <div class="tw-flex tw-flex-col tw-justify-around tw-text-center tw-text-orange tw-border-2 tw-border-orange-light tw-rounded tw-h-full tw-py-0 tw-px-1">
                                ...
                                <p class="sm:tw-text-xl tw-text-lg tw-font-light roboto-slab sm:tw-py-3 tw-py-1">
                                    Hot!
                                </p>
                            </div>
                        </a>
                ...
                @endif
                <div class="xl:tw-flex-col tw-flex tw-leading-tight tw-w-5/6 sm:tw-ml-6 tw-ml-4">
                    <div class="tw-flex-grow tw-pr-4 coupon-card-inner-container">
                        <a id="testingId3"
                           dusk="coupon-card-label"
                           href="#!"
                           rel="nofollow"
                    ...
                    <div class="tw-hidden md:tw-flex tw-items-start tw-whitespace-no-wrap tw-min-w-1/4">
                        <div rel="nofollow"
                             class="tw-relative tw-rounded-sm tw-overflow-hidden tw-cursor-pointer tw-w-3/4 lato">
                            @if($coupon->code)
                                <div class="tw-bg-grey-lighter tw-border tw-text-grey-dark tw-text-right tw-w-full tw-p-2 tw-pr-4">
                                    {{ $coupon->code }}
                                </div>
